...
// Total number of response times to store for a given task. These values are used
// to calculate the moving average of the response time.  Note that the higher the number,
// the more accurate the average will be.
// See http://en.wikipedia.org/wiki/Moving_average#Simple_moving_average
static NSInteger const kSLNNumberOfReponseTimesToInclude = 3;
...
- (void)addResponseTime:(CGFloat)responseTime {
  ...
  // Only keep the last kSLNNumberOfReponseTimesToInclude
  if ([self.recentReponseTimes count] > kSLNNumberOfReponseTimesToInclude) {
...
- (NSString *)description {
  return [NSString stringWithFormat:@"task: %@, last execution time: %f, moving average response time: %f", [self key], self.lastExecutionTime, [self movingAverageResponseTime]];
}
...
//
static inline CGFloat Normalize(CGFloat value, CGFloat min, CGFloat max) {
...
//
static inline CGFloat Score(SLNTaskPriority priority, SLNTaskPriority minPriority, SLNTaskPriority maxPriority, CGFloat time, CGFloat minTime, CGFloat maxTime) {
  return Normalize(priority, minPriority, maxPriority) * Normalize(time, minTime, maxTime);
};
...
//
- (NSArray *)nextTasks {
  __block CGFloat minTimeSinceLastExecution = 0.0;
  __block CGFloat maxTimeSinceLastExecution = 0.0;
  __block SLNTaskPriority minPriority = SLNTaskPriorityVeryLow;
  __block SLNTaskPriority maxPriority = SLNTaskPriorityVeryLow;
  
  // Calculate the min/max values
  [self.taskContainers enumerateObjectsUsingBlock:^(SLNTaskContainer *t, NSUInteger idx, BOOL *stop) {
    minTimeSinceLastExecution = MIN(t.elapsedTimeSinceLastExecution, minTimeSinceLastExecution);
    maxTimeSinceLastExecution = MAX(t.elapsedTimeSinceLastExecution, maxTimeSinceLastExecution);
    minPriority = MIN([t.task priority], minPriority);
    maxPriority = MAX([t.task priority], maxPriority);
  }];
  
  // Calculate the score for every task
  [self.taskContainers enumerateObjectsUsingBlock:^(SLNTaskContainer *t, NSUInteger idx, BOOL *stop) {
    t.score = Score([t.task priority], minPriority, maxPriority, [t elapsedTimeSinceLastExecution], minTimeSinceLastExecution, maxTimeSinceLastExecution);
  }];
  ...
  CGFloat totalResponseTime = 0.0;
  
  NSMutableArray *scheduledTasks = [[NSMutableArray alloc] init];
  
  int index = 0;
  while ((totalResponseTime < kSLNAvailableTime) && (index < [sortedTasksByScore count])) {
    SLNTaskContainer *t = (SLNTaskContainer *)sortedTasksByScore[index];
    totalResponseTime += [t movingAverageResponseTime];
